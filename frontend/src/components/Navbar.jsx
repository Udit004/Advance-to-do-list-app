    ...
    const fetchUserProfile = async () => {
      if (currentUser) {
        try {
          ...
          setUserProfile(currentUser); // Fallback to original user if profile fetch fails
        }
      } else {
        setUserProfile(null);
      }
      ...
      <div className="container flex mx-auto px-4">
        ...
        <div className="flex items-center flex-grow">
          <ul className="flex mx-auto space-x-2">
            <li>
              <Link
                className="px-3 py-2 text-white hover:text-cyan-300 transition-colors font-medium"
                to="/"
              >
                Home
              </Link>
          ...
          <div className="flex z-50 items-center gap-4">
            ...
            {!currentUser ? (
              <div className="flex items-center gap-3">
                <Link
                  to="/login"
                  className="px-4 py-2 border border-slate-300/20 text-slate-200 hover:bg-white/10 rounded-lg transition-all duration-300 ease-out hover:scale-105"
                >
                  Sign in
                </Link>
                <Link
                  to="/signup"
                  className="px-4 py-2 bg-gradient-to-r from-cyan-500 via-blue-500 to-purple-500 hover:from-cyan-400 hover:via-blue-400 hover:to-purple-400 text-white rounded-lg shadow-lg shadow-blue-500/20 hover:shadow-blue-500/40 transition-all duration-300 ease-out hover:scale-105"
                >
                  Sign up free
                </Link>
              </div>
              ...
              <div className=" z-50 relative group">
                ...
                <div className="z-10000 absolute right-0 mt-2 w-48 bg-slate-900/95 backdrop-blur-xl rounded-lg shadow-xl shadow-black/20 py-1 border border-white/10 invisible group-hover:visible transition-all opacity-0 group-hover:opacity-100">
                  <Link
                    className="block px-4 py-2 text-slate-200 hover:bg-white/10 hover:text-white transition-colors"
                    to="/profile"
                  >
                    Profile
                  </Link>
