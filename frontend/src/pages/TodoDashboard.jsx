...
import { List, FolderOpen, Menu, X, Sparkles, Lock, Crown, ArrowRight, Home } from "lucide-react";
...
const ProtectedFeature = ({ children, isPaid, isLoading, featureName }) => {
  ...
  return (
    <div className="relative h-full">
      ...
      <div className={`h-full transition-all duration-300 ${!isPaid ? 'filter blur-[2px] sm:blur-[4px] pointer-events-none select-none' : ''}`}>
        {children}
      </div>
...
const MobileBottomNav = ({ activeItem, setActiveItem, userProfile, isLoading }) => {
  const menuItems = [
    {
      id: "todoList",
      label: "Tasks",
...
const Sidebar = ({ isExpanded, setIsExpanded, activeItem, setActiveItem, userProfile, isLoading }) => {
  ...
  return (
    <>
      ...
      <div
        className={`
          fixed md:relative z-40 h-full bg-gradient-to-b from-slate-900 to-slate-800 shadow-xl flex flex-col 
          transition-all duration-300 ease-in-out
          ${isExpanded ? "w-72 sm:w-80 md:w-64" : "w-0 md:w-16"}
      ...
      >
        ...
        <div className="p-4 border-b border-slate-700/50 flex items-center justify-between min-h-[64px]">
          ...
          <button
            ...
            className="p-2 rounded-lg hover:bg-slate-700/50 active:bg-slate-700/70 text-slate-300 hover:text-white transition-all duration-200 touch-manipulation"
          >
          ...
          {menuItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`
                  w-full flex items-center space-x-3 px-4 py-3 sm:py-4 rounded-xl
              ...
              >
                ...
                {isExpanded && (
                  <div className="flex items-center justify-between flex-1">
                    <span className="font-medium text-sm sm:text-base transition-opacity duration-300">
                      {item.label}
                    </span>
                    {isPremiumFeature && (
                ...
                {!isExpanded && (
                  <div className="absolute left-full ml-2 px-3 py-1 bg-slate-800 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-10 shadow-lg hidden md:block">
                    {item.label}
                    {isPremiumFeature && (
              ...
              {!userProfile?.isPaid && !isLoading && (
                <div className="space-y-2">
                  <button
                    ...
                    className="w-full bg-gradient-to-r from-blue-600 to-purple-600 text-white py-3 sm:py-4 px-4 rounded-lg font-semibold text-sm sm:text-base hover:from-blue-700 hover:to-purple-700 active:from-blue-800 active:to-purple-800 transition-all duration-200 transform active:scale-95 shadow-lg touch-manipulation"
                  >
                    <div className="flex items-center justify-center space-x-2">
                      <Crown size={16} />
...
const MobileHeader = ({ isExpanded, setIsExpanded, userProfile, activeItem }) => {
  const getPageTitle = () => {
    switch (activeItem) {
      case "todoList":
        return "Todo List";
  ...
  return (
    <div className="bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between md:hidden">
      <div className="flex items-center space-x-3">
        <button
          onClick={() => setIsExpanded(!isExpanded)}
          className="p-2 rounded-lg hover:bg-gray-100 active:bg-gray-200 transition-all duration-200 touch-manipulation"
        >
        ...
        <div className="flex items-center justify-center min-h-full p-4">
          <div className="text-center max-w-sm">
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 sm:p-6 mb-4">
              <h3 className="text-yellow-800 font-semibold mb-2 text-base sm:text-lg">Authentication Required</h3>
        ...
        <div className="flex items-center justify-center min-h-full p-4">
          <div className="text-center max-w-sm">
            <div className="bg-red-50 border border-red-200 rounded-lg p-4 sm:p-6 mb-4">
              <h3 className="text-red-800 font-semibold mb-2 text-base sm:text-lg">Connection Error</h3>
        ...
        return (
          <div className="p-4 sm:p-6 lg:p-8">
            ...
            <p className="text-gray-600 text-sm sm:text-base mb-4">
              Select a menu item to get started with your productivity dashboard.
            </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
              ...
              >
                <div className="flex items-center mb-2">
                  ...
                  {!userProfile?.isPaid && <Lock className="w-4 h-4 text-gray-500" />}
                </div>
                <h3 className="font-medium text-gray-800">Project Dashboard</h3>
                <p className="text-sm text-gray-600">
                  {userProfile?.isPaid ? "Organize complex projects" : "Premium feature"}
                </p>
              </button>
            </div>
