...
// SINGLE PROJECT ROUTES (using :id for consistency)
...
router.patch(
  "/:projectId/collaborators/:collaboratorId/role",
  async (req, res) => {
    try {
...
router.post("/:projectId/join", async (req, res) => {
  try {
    ...
    if (!project.isPublic) {
      return res
        .status(403)
        .json({ success: false, message: "Project is private" });
    }
    ...
    res
      .status(200)
      .json({ success: true, message: "Successfully joined project" });
  } catch (error) {
